...
module PopUpManager {
	...
	* duration          动画持续时间
    */
	export function addPopUp(panel, dark: boolean = false, popUpWidth: number = 0, popUpHeight: number = 0, effectType: number = 0, isAlert: boolean = false, duration: number = 500, cb?: Function): void {
		...
		//以下是弹窗动画
		var leftX: number = GlobalConfig.curWidth() / 2 - popUpWidth / 2;
		var upY: number = GlobalConfig.curHeight() / 2 - popUpHeight / 2;
